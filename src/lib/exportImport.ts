import type { ExportBundle, Person, Template, ORBAT } from '../types'
...
export function generateFilename(type: 'people' | 'templates' | 'orbats' | 'all'): string {
  const date = new Date().toISOString().split('T')[0]
  return `orbat-maker-${type}-${date}.json`
}

export function parseImportFile(text: string): { bundle: ExportBundle; error?: string } {
  try {
    const data = JSON.parse(text)
    if (!data.version || !data.exportedAt) {
      return { bundle: data, error: 'Invalid file: missing version or exportedAt' }
    }
    return { bundle: data }
  } catch {
...
export function describeBundle(bundle: ExportBundle): string {
  ...
  if (bundle.templates?.length) parts.push(`${bundle.templates.length} templates`)
