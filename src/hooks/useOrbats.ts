      ...
      setOrbats((prev) =>
        prev.map((o) => {
          ...
          const updated = o.assignments.map((a) => {
            if (a.slotId === slotIdA && assignB)
              return { slotId: slotIdA, personId: assignB.personId };
            if (a.slotId === slotIdB && assignA)
              return { slotId: slotIdB, personId: assignA.personId };
            return a;
          });
  ...
  const movePersonToSlot = useCallback(
    (orbatId: string, sourceSlotId: string, targetSlotId: string) => {
      setOrbats((prev) =>
        prev.map((o) => {
          if (o.id !== orbatId) return o;
          const assignment = o.assignments.find(
            (a) => a.slotId === sourceSlotId,
          );
          ...
          return {
            ...
            assignments: [
              ...filtered,
              { slotId: targetSlotId, personId: assignment.personId },
            ],
          };
        }),
      );
    ...
    (orbatId: string, slotId: string) => {
      setOrbats((prev) =>
        prev.map((o) => {
          ...
          return {
            ...o,
            assignments: o.assignments.filter((a) => a.slotId !== slotId),
          };
        }),
      );
    },
