...
import { Plus, Trash2, FolderOpen, ClipboardList } from 'lucide-react'
...
export function OrbatListPage({ onNavigate }: OrbatListPageProps) {
  ...
  function getAssignmentProgress(orbatId: string): { filled: number; total: number } {
  ...
  return (
    <div>
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <ClipboardList size={16} className="text-green-400 flex-shrink-0 mt-0.5" />
                    <span className="font-semibold text-gray-200 truncate">{orbat.name}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-1 text-xs text-gray-500">
                  <span className="text-gray-400 truncate">
                    {template ? template.name : <span className="text-yellow-400/80">Template missing</span>}
                  </span>
                  {orbat.date && (
                    <span>{formatDate(orbat.date)}</span>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className={filled === total && total > 0 ? 'text-green-400' : 'text-gray-400'}>
                      {filled}/{total}
                    </span>
                  </div>
                  {total > 0 && (
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-[#2a2a4a] mt-auto">
                  ...
                  <Button
                    variant="danger"
                    size="sm"
                    onClick={() => setDeleteTarget({ id: orbat.id, name: orbat.name })}
                    title="Delete ORBAT"
                  >
                    <Trash2 size={13} />
                  </Button>
      ...
      {/* New ORBAT modal */}
      <Modal open={showNewModal} onClose={handleCloseNewModal} title="New ORBAT">
        <div className="flex flex-col gap-4">
          <TextInput
            ...
            onKeyDown={e => { if (e.key === 'Enter' && canCreate) handleCreateOrbat() }}
            autoFocus
          />
