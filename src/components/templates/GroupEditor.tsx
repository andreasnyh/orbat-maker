...
export function GroupEditor({ group, onUpdate, onDelete, dragHandleProps }: GroupEditorProps) {
  ...
  function updateSlot(updated: Slot) {
    onUpdate({ ...group, slots: group.slots.map(s => (s.id === updated.id ? updated : s)) })
  }
      ...
      <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg overflow-hidden">
          ...
          <div className="flex-1 min-w-0">
            {editingName ? (
              ...
              <span
                ...
                className="block text-sm font-semibold text-gray-200 cursor-text hover:text-white truncate"
        ...
        <div className="px-1 py-1">
          {group.slots.length > 0 ? (
            ...
            <p className="text-xs text-gray-600 text-center py-2 italic">No slots — add one below</p>
          )}
        </div>
