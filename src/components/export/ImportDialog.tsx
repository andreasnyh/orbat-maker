...
import { parseImportFile, describeBundle } from '../../lib/exportImport'
...
export function ImportDialog({ open, onClose }: ImportDialogProps) {
  const { people, setPeople, templates, setTemplates, orbats, setOrbats } = useAppState()
  ...
  function handleImport() {
    ...
    if (bundle.templates?.length) {
      ...
      const newTemplates = bundle.templates.filter(t => !existingIds.has(t.id))
        ...
        {(state.phase === 'idle' || state.phase === 'error') && (
          <>
            <p className="text-sm text-gray-400">
              Select an ORBAT Maker JSON export file. Existing records with matching IDs will be
              skipped; new records will be merged in.
            </p>
        ...
        {state.phase === 'preview' && (
          <>
            <div className="bg-[#0f0f23] border border-[#2a2a4a] rounded-md p-4 flex flex-col gap-2">
              <p className="text-xs text-gray-500 font-mono truncate">{state.filename}</p>
              <p className="text-sm text-gray-300">
                This file contains:{' '}
                <span className="text-green-400 font-medium">{describeBundle(state.bundle)}</span>
              </p>
            ...
            <p className="text-xs text-gray-500">
              Import mode: <span className="text-gray-300">Merge</span> — records whose IDs already
              exist in the app will be skipped.
            </p>
        ...
        {state.phase === 'success' && (
          <>
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-sm text-gray-400">{state.description} merged successfully.</p>
            </div>
